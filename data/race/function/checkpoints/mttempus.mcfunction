...
# Surplus Items
# execute unless score #mtTempus checkpoints matches 1.. run function race:items/amaria
...
# Catchup
effect give @a[tag=!mtTempus] speed 40 1 false
execute if score #mode gamemode matches 2 at @s as @e[type=skeleton_horse,sort=nearest,limit=1] run tag @s add mtTempus
execute if score #mode gamemode matches 2 run effect give @e[type=skeleton_horse,tag=!mtTempus] speed 40 1 false