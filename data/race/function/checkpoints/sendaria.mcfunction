...
# Surplus Items
# execute unless score #sendaria checkpoints matches 1.. run function race:items/sendaria

# Tags and Scoreboards
scoreboard players set #sendaria checkpoints 1
tag @s add sendaria
...
execute if score #mode gamemode matches 2 at @s as @e[type=skeleton_horse,sort=nearest,limit=1] run tag @s add sendaria
execute if score #mode gamemode matches 2 run effect give @e[type=skeleton_horse,tag=!sendaria] speed 40 1 false