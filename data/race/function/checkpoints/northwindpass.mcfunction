...
#tellraw @a [{"selector":"@s","color":"dark_green"},{"text":" has reached ","color":"gold"},{"text":"[Northwind Pass]","color":"green","hover_event":{"action":"show_text","value":{"text":"Reach Northwind Pass","color":"green"}}}]
advancement grant @s only race:northwindpass
execute as @a at @s run playsound minecraft:entity.experience_orb.pickup master @s

# Catchup
effect give @a[tag=!northwindPass] speed 40 1 false
execute if score #mode gamemode matches 2 at @s as @e[type=skeleton_horse,sort=nearest,limit=1] run tag @s add northwindPass
execute if score #mode gamemode matches 2 run effect give @e[type=skeleton_horse,tag=!northwindPass] speed 40 1 false